...
<INPUT TYPE=hidden NAME="register" VALUE="SignUp">
...
def registerUser(uname, passwrd):
    ...
    t = (uname,passwrd)
    c.execute('INSERT INTO users VALUES(?,?)',(uname, passwrd))
    ...
    if "action" in form:
    	...
    	if action == "SignUp":
            ...
            if "pictures" in form:
            	piture = form["picture"]
            	if picture.file:
            		f = open(os.enviorn["DOCUMENT_ROOT"] + "/PictureShare/" + picture.filename, "w")
